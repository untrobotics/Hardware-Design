...
Stepper myStepper(stepsPerRevolution, 8, 9, 10, 11); // initialize the stepper library on pins 8 through 11 //Recall pins 9 through 11 are PWM 
...
void loop() {
  ...
  /*   -------BOUNUS-------- 
  // step one revolution in the other direction and will be at full speed 
  myStepper.setSpeed(full);  // set the speed at 60 rpm or full speed
  ...
  //step one revolution in one direction and will be at half speed
  myStepper.setSpeed(half); // reduce speed by half 
  Serial.println("anticlockwise: HALF SPEED"); //print to Serial monitor 
  myStepper.step(-stepsPerRevolution); //Detakes the direction of the stepper motor
  delay(2000); //delay function is in miliseconds 
